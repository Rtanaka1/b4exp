...
impl<'tcx> UninitExposureChecker<'tcx> {
    ...
    pub fn analyze(self) {
        ...
        for (_ty_hir_id, (body_id, related_item_span)) in self.rcx.types_with_related_items() {
            ...
            if let Some(status) = inner::UninitExposureBodyAnalyzer::analyze_body(self.rcx, body_id, visible, tcx.def_path_str(body_did))
            {
                let behavior_flag = status.behavior_flag();
                if !behavior_flag.is_empty()
                {
                    if !crate::analysis::llm_triage::evaluate_bug(tcx, "Uninit Exposure Bug", body_did, None) {
                        continue;
                    }
...
mod inner {
    ...
    impl<'a, 'tcx> UninitExposureBodyAnalyzer<'a, 'tcx> {
        ...
        pub fn analyze_body(rcx: TypePulseCtxt<'tcx>, body_id: BodyId, visible: bool, fn_sym: String) -> Option<UninitExposureStatus> {
                ...
                if let Unsafety::Unsafe = fn_sig.unsafety() {
                    ...
                    match rc_body.as_ref() {
                        ...
                        Ok(body) => {
                            ...
                            Some(body_analyzer.analyze(rc_call_graph.as_ref(), body_did, visible, fn_sym))
                        }
                    }
                }
            } else {
        ...
        fn analyze(mut self, call_graph: &CallGraph<'tcx>, cur_func_id: DefId, visible: bool, fn_sym: String) -> UninitExposureStatus {
                                                            ...
                                                            if is_from_gen == true && is_to_gen == false && tty.to_string() != "usize" && !tty.is_c_void(tcx) && !tty.contains(fty) {
                                                                ...
                                                                if !is_from_trans && !is_from_c {
                                                                    ...
                                                                    self.status
                                                                        .ty_convs
                                                                        .push(statement.source_info.span);
                                                                    // rely on function name to identify iterator function, which should be exposed
                                                                    if fn_sym.contains("as std::iter::Iterator>::next") {
                                                    ...
                                                    match pl {
                                                        Ok(place) => {
                                                            ...
                                                            if is_from_gen == true && is_to_gen == false && tty.to_string() != "usize" && !tty.is_c_void(tcx) && !tty.contains(fty) {
                                                                ...
                                                                if !is_from_trans && !is_from_c && !fty.is_c_void(tcx) && !from_ty_simd {
