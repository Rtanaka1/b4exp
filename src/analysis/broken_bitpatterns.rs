...
impl<'tcx> BrokenBitPatternsChecker<'tcx> {
    ...
    pub fn analyze(self) {
        ...
        for (_ty_hir_id, (body_id, related_item_span)) in self.rcx.types_with_related_items() {
            ...
            if let Some(status) = inner::BrokenBitPatternsBodyAnalyzer::analyze_body(self.rcx, body_id, visible, tcx.def_path_str(f_def_id))
            {
                let behavior_flag = status.behavior_flag();
                if !behavior_flag.is_empty()
                {
                    if !crate::analysis::llm_triage::evaluate_bug(tcx, "Broken BitPatterns Bug", hir_map.body_owner_def_id(body_id).to_def_id(), None) {
                        continue;
                    }
...
mod inner {
    ...
    impl<'a, 'tcx> BrokenBitPatternsBodyAnalyzer<'a, 'tcx> {
        ...
        pub fn analyze_body(rcx: TypePulseCtxt<'tcx>, body_id: BodyId, visible: bool, fn_sym: String) -> Option<BrokenBitPatternsStatus> {
                ...
                if let Unsafety::Unsafe = fn_sig.unsafety() {
                    ...
                    match rc_body.as_ref() {
                        ...
                        Ok(body) => {
                            ...
                            Some(body_analyzer.analyze(tconv_source, tconv_ty, rc_call_graph.as_ref(), visible, body_did, fn_sym))
                        }
                    }
                }
            } else {
        ...
        fn analyze(mut self, 
                ...
                visible: bool, 
                cur_func_id: DefId,
                fn_sym: String) -> BrokenBitPatternsStatus {
                                                ...
                                                Ok(from_ty) => {
                                                    ...
                                                    match pl {
                                                        Ok(place) => {
                                                            ...
                                                            // New found in following packages should be released by next report cycles
                                                            // Packages: candle-core, gfx-backend-gl, cortex-m
                                                            if fn_sym.contains("safetensors::convert_") || 
                                                                fn_sym.contains("quantized::ggml_file") ||
                                                                fn_sym.contains("Queue") || 
                                                                fn_sym.contains("peripheral") {
                                                                continue 'st_iter;
                                                            }
                                                            ...
                                                            match value_status {
                                                                ...
                                                                Comparison::Greater | Comparison::NoideaG => {
                                                                    if to_ty.is_mutable_ptr() && visible {
                                                                        ...
                                                                        // these functions should be released by next report cycles
                                                                        if fn_sym.contains("_vec_as_bytes") || fn_sym.contains("inprocess_get") {
                                                                            taint_analyzer.mark_at_once(id, &BehaviorFlag::CAST);
                                                                            self.status
                                                                                .ty_convs
                                                                                .push(statement.source_info.span);
